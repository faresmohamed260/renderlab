...
contract = r'''# Phase 24 — Library Gallery Rail v0.3 Implementation Contract
...
**Decision:** The user-approved Gallery Rail v0.3 direction is the visual/interaction authority for the next `/library` production implementation. Library keeps the UI-074 compact horizontal application header, compresses route context, promotes search into the primary retrieval field, consolidates source/filter/organization controls into one compact command rail, and keeps the media field visually dominant. Current-page selection transforms that same command rail in place into selection summary/actions; it must not introduce a generic floating batch toolbar over media.

**Product boundary:** All current Library contracts remain unchanged: Creatives/Uploads are URL/server-owned views over the same durable `media_assets`; kind/search/Favorites/Collections/sort/pagination remain URL/server-owned; uploads and desktop drop stay Uploads-only; selection remains current-page transient state with existing bounded Organize/Delete behavior; cards remain Viewer links; account ownership/security and durable-media identity remain unchanged. No schema, API, worker/provider, storage, routing or deployment change is authorized.

**Card/interaction boundary:** UI-075 supersedes UI-070's card presentation only where the approved v0.3 evidence differs. UI-070's maintained 44×44 checkbox root, centered 22×22 visible indicator, card-link semantics, no quick-actions rule, no hover-only essential behavior and no client-owned media state remain authoritative. Pointer-capable media may use bounded local depth/spotlight; touch and reduced motion keep complete static meaning. Ordinary command controls remain stable.

**Truthfulness rule:** Prototype counts and labels are representative design content, not product facts. Production may show only counts/labels supported by real route data; sort/source context must reflect active URL state. Do not fabricate total corpus size, progress or capabilities for visual fidelity.

**Evidence:** user-approved R&D head `ba842e919305e07262ae95c81b3c2063a455b54d`; exact-head run `34689164957` passed; artifact `10296866215`, digest `sha256:db81c13f34514022f01a9e8998de4c0622ee946e316d02655c70477db1a87d95`. Issue #191 records the audit, three-concept study, professional critique/refinement and explicit user approval. Phase 24 implementation is tracked by #192 and governed by `docs/ui/LIBRARY_GALLERY_RAIL_IMPLEMENTATION_CONTRACT.md`.

**Consequence:** This decision approves the design target and implementation contract only. It does not claim the production Library has changed and does not authorize deployment. The isolated R&D branch stays unmerged design evidence.
'''
...
contract_path = Path('docs/ui/LIBRARY_GALLERY_RAIL_IMPLEMENTATION_CONTRACT.md')
if contract_path.exists():
    raise SystemExit('contract already exists')
contract_path.write_text(contract, encoding='utf-8')

decisions_path = Path('docs/ui/UI_DECISIONS.md')
decisions = decisions_path.read_text(encoding='utf-8')
if '### UI-075' in decisions:
    raise SystemExit('UI-075 already exists')
decisions_path.write_text(decisions.rstrip() + ui075 + '\n', encoding='utf-8')

migration_path = Path('docs/ui/UI_MIGRATION.md')
migration_text = migration_path.read_text(encoding='utf-8')
marker = '**Phase 23 correction status: `USER-APPROVED / EXACT-HEAD VERIFIED / MERGED / MERGED-MAIN VERIFIED / NOT DEPLOYED`.**'
if marker not in migration_text:
    raise SystemExit('UI_MIGRATION marker missing')
migration_text = migration_text.replace(marker, marker + migration, 1)
migration_path.write_text(migration_text, encoding='utf-8')

registry_path = Path('docs/ui/SCREEN_REGISTRY.md')
registry = registry_path.read_text(encoding='utf-8')
artifacts_marker = '**Approved design artifacts:** `design/penpot/library-v0.1.svg`, `design/penpot/library-v0.2-upload.svg`'
if artifacts_marker not in registry:
    raise SystemExit('SCREEN_REGISTRY Library marker missing')
registry_insert = artifacts_marker + '\n**Phase 24 redesign:** USER-APPROVED DESIGN / IMPLEMENTATION PENDING. Authority: Gallery Rail v0.3 R&D head `ba842e919305e07262ae95c81b3c2063a455b54d`, artifact `10296866215`; production contract: `docs/ui/LIBRARY_GALLERY_RAIL_IMPLEMENTATION_CONTRACT.md`; implementation tracker: #192.'
registry = registry.replace(artifacts_marker, registry_insert, 1)
library_rule = '**Do not change:** Do not couple Library to legacy `studio_*` or expose temporary `generation_sources` as durable media.'
if library_rule not in registry:
    raise SystemExit('SCREEN_REGISTRY Library do-not-change marker missing')
registry = registry.replace(library_rule, '**Phase 24 visual direction:** Gallery Rail v0.3 may replace the current stacked Library command composition and the UI-070 card presentation only to the extent defined by UI-075. It must preserve maintained 44×44 / 22×22 selection geometry, card→Viewer activation, URL/server-owned discovery, current-page batch semantics, Uploads-only upload/drop and all ownership/security contracts.\n\n' + library_rule, 1)
registry_path.write_text(registry, encoding='utf-8')

project_path = Path('PROJECT.md')
project_text = project_path.read_text(encoding='utf-8')
project_marker = '## Current Verified Baseline — 2026-09-12'
if project_marker not in project_text:
    raise SystemExit('PROJECT marker missing')
project_text = project_text.replace(project_marker, project + '\n' + project_marker, 1)
