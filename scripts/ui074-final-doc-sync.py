...
HEAD = "c786a17fa3a3c7f76dba5a64cb7822926749c1a2"
...
text = replace_once(
    text,
    r"\*\*Phase 23 fidelity-correction candidate — merge pending:\*\*.*?\n\n",
    shell + "\n\n",
...
p = Path("docs/ui/COMPONENT_CATALOG.md")
text = p.read_text(encoding="utf-8")
...
text = replace_once(
    text,
    r"\*\*Phase 23 / UI-074 fidelity-correction candidate:\*\*.*?\n\n",
    shell + "\n\n",
...
p = Path("docs/ui/CREATE_CLEAR_COMPOSER_IMPLEMENTATION_CONTRACT.md")
text = p.read_text(encoding="utf-8")
...
text = replace_once(
    ...
    block.rstrip() + "\n",
    "Create contract correction block",
)
write(p, text)
