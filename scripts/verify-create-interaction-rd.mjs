...
try {
  ...
  await reduced.screenshot({ path: path.join(out, 'mobile-result-reduced.png'), fullPage: true });
  assert(motionCss !== undefined, 'Reduced-motion computed style could not be read.');

  console.log('Create interaction R&D verified: desktop/mobile states, pointer reorder, keyboard activation/focus, reduced-motion path, and temporal capture passed.');
} finally {
  await browser.close();
}
