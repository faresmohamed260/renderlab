...
async function createVideoMediaAsset(account, {
  id = randomUUID(),
  displayName,
  generationJobId = null,
  generationOutputIndex = null,
...
try {
  ...
  const compareVideoResultId = randomUUID();
  ...
  const temporaryResult = await createMediaAsset(owner, {
  ...
  await page.goto(`${baseUrl}/library/${compareVideoResultId}`, { waitUntil: "networkidle", timeout: 60_000 });
