...
- `media-viewer-v0.3-upscale-2x-desktop.svg`, `media-viewer-v0.3-upscale-2x-mobile.svg`, `media-viewer-v0.3-upscale-2x.md` — **REVIEWED DESIGN CANDIDATE / UI-058 Phase 18E repository-backed checkpoint**. The candidate preserves the existing media-first Viewer and UI-056 comparison hierarchy, keeps Edit primary + Animate secondary, places server-eligible `Upscale 2×` on a full-width secondary Continue row, and records stable starting/accepted feedback with `Open Activity` at desktop and 390px narrow width. It is a design checkpoint, not implementation approval; real GitHub/Playwright rendered verification remains required.
