...
- `library-v0.1.svg` — Phase 4 Library candidate showing the media-first newest grid, `All / Images / Videos` filtering, responsive two-column mobile layout, and truthful empty-state direction. It intentionally omits favorites, collections, batch actions, density controls and legacy Creatives/Uploads tabs until RenderLab owns those contracts.
- `media-viewer-v0.1.svg` — Phase 4 contextual Media Viewer candidate showing media-primary desktop/mobile layout, secondary prompt/details metadata, and the intended Edit/Animate continuation placement. Continuation buttons are a design target only until the cross-route durable-media handoff to Create is implemented and verified.
