...
- `library-v0.2-upload.svg` — repository-backed handoff for the scoped persistent-upload extension: one compact Upload action integrated into the existing Library header, native file picker, existing grid as the durable surface, and concise inline upload/error feedback. This artifact does **not** approve a separate Uploads tab, generic upload modal, favorites, collections, rename/delete/batch management or drag/drop. The implementation exists in PR #9 but is **not yet approved** because the configured browser direct-R2 lifecycle is blocked by shared-bucket CORS and has not produced inspectable desktop/mobile screenshots.
...
Durable product rules are documented in `docs/ui/UI_DECISIONS.md`. Create behavior is governed especially by UI-015 through UI-019; initial Library/Media Viewer scope by UI-020/UI-021; persistent uploaded-media identity by UI-022. Phase status is tracked in `docs/ui/UI_MIGRATION.md`.
