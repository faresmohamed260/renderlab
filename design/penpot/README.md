# Penpot Design Handoff

This directory contains open, versioned visual artifacts that can be imported into Penpot and reviewed without depending on Figma or a proprietary design-tool connector.

## Current files
- `create-v0.2-desktop.svg` — reviewed Create v0.2 desktop states: Create Image, Edit Image, Create Video.
- `create-v0.2-mobile.svg` — revised Create v0.2 narrow/mobile candidate using a two-row composer.
- `create-v0.3-advanced.svg` — reviewed desktop/mobile Advanced-disclosure candidate. The panel stays inside the composer, is collapsed by default, and exposes only currently verified advanced generation parameters.
- `library-v0.1.svg` — approved Phase 4 Library v0.1 direction: media-first newest grid, `All / Images / Videos` filtering, responsive two-column mobile layout and truthful empty states. Favorites, collections, batch actions, density controls and legacy Creatives/Uploads tabs remain intentionally omitted.
- `library-v0.2-upload.svg` — approved repository-backed handoff for the scoped persistent-upload extension: one compact Upload action integrated into the existing Library header, native file picker, existing grid as the durable surface, and concise inline upload/error feedback. It does **not** approve a separate Uploads tab, generic upload modal, favorites, collections, rename/delete/batch management or drag/drop. The implemented extension passed configured browser Upload → Library → Viewer → Create verification and desktop/mobile screenshot review before PR #9 merged.
- `media-viewer-v0.2-compare-source-desktop-image.svg`, `media-viewer-v0.2-compare-source-desktop-video.svg`, `media-viewer-v0.2-compare-source-mobile-image.svg`, `media-viewer-v0.2-compare-source-mobile-video.svg` — **REVIEWED DESIGN CANDIDATE / UI-056 Phase 16D checkpoint accepted 2026-09-05**. The approved direction preserves the default Viewer, keeps Result primary, uses truthful media geometry, a compact contextual narrow Source card, `Open source`, existing result-owned actions/video controls and `Close comparison`. These artifacts authorize the bounded implementation direction but are not themselves the rendered implementation source of truth.
- `media-viewer-v0.3-upscale-2x-desktop.svg`, `media-viewer-v0.3-upscale-2x-mobile.svg`, `media-viewer-v0.3-upscale-2x.md` — **UI-058 Phase 18E checkpoint / IMPLEMENTED / RENDER VERIFIED**. The accepted candidate preserves the existing media-first Viewer and UI-056 comparison hierarchy, keeps Edit primary + Animate secondary, and places server-eligible `Upscale 2×` on a full-width secondary Continue row with stable starting/accepted/error feedback. Implementation head `ac4aed60e64061ee6a911c858cdc032b6f9a7423` passed Upscale Viewer Visual `34033506667`; artifact `9989427506` (`sha256:49e40583899891a1f1863ff8fd49a714febf11bfdb999bb781849f45c3e01121`) was hash-checked and human-reviewed clean at desktop and 390px narrow width with no corrective UI change. The artifacts remain design handoff evidence; repository code/docs remain authoritative.
- `media-viewer-v0.1.svg` — approved Phase 4 contextual Media Viewer v0.1 handoff showing media-primary desktop/mobile layout, secondary prompt/details metadata and Edit/Animate continuation placement. The implementation and server-validated durable-media handoff to Create were approved after configured run `33034606396`; uploaded-media presentation/continuation was approved through PR #9. Download v0.1 (UI-024) and Rename v0.1 (UI-025) are deliberately incremental Viewer actions that preserve this base layout rather than creating a replacement design artifact. Download merged through PR #11; Rename's refined desktop/mobile edit/renamed screenshots from `33074480356` were visually inspected, including the final composition where Rename and Download remain side-by-side and the inline editor expands beneath them.
- `brand-launch-v0.1-desktop.svg` — **REVIEWED DESIGN CANDIDATE** for UI-052 at the contract desktop review size `1440×1100`. It establishes the geometric RenderLab `R` mark, marketing navigation, truthful closed-beta hero/CTAs, a real-capability Create montage and the four verified creative operations without introducing pricing, testimonials, fake metrics, provider claims or public signup.
- `brand-launch-v0.1-mobile.svg` — **REVIEWED DESIGN CANDIDATE** for UI-052 at the contract narrow review size `390×844`. It preserves the same mark/copy/CTA hierarchy and a compact real-capability Create preview without horizontal pressure or desktop-chrome imitation.

## Phase 11 reviewed direction
The Phase 11 candidate deliberately stays inside the accepted RenderLab visual family: dark canvas/surfaces, one semantic violet accent, restrained borders and Inter/system sans-serif. The original geometric `R` mark is a simple monochrome-capable stem/bowl/diagonal construction rather than a sparkle, glow or gradient-dependent AI glyph. Marketing uses larger type and more whitespace than the application, while the product preview uses only current RenderLab concepts such as Image/Video intent, `@image1`, durable references, Generate, Library continuation and the verified Create/Edit/Video/Animate operation set.

Desktop and narrow candidates were visually reviewed together before Phase 11 route or React implementation. Both retain reachable `Open Create` and `Sign in` actions, explicit invitation-only language and a clear visual relationship to the existing application without importing marketing decoration into `AppShell`. The SVG pair remains a **REVIEWED DESIGN CANDIDATE** rather than the application source of truth. The matching PR #73 implementation at validated head `8975b7b42b518eea0a462b28528ddd41d90ad986` passed Brand / Launch Visual `33321365147` and all 19 affected workflows; artifact `9734984885` (`sha256:8d9929fb5f6d85da4710184ec7bbe756f782593f58525ec2ae660729ad3b32a9`) was human-reviewed clean for landing desktop/narrow, `/create` shell, small-scale mark and legacy continuation. The rendered implementation merged through PR #73 as `46c5daa2866c6758907ee9be219bcb3cb274ca83` after final head `773251734dbd5c5f32770699a57b1ade653604b5` passed 19/19 affected workflows; merged-main UI Shell `33323421285` also passed. The design-artifact authority order is unchanged.

## Authority
These SVGs are design handoff artifacts, not the project source of truth.

Authority order remains:
1. repository documentation/code;
2. current Penpot workspace when one is available;
3. repository design handoff artifacts in this directory;
4. historical Figma material;
5. chat/session context.

Durable product rules are documented in `docs/ui/UI_DECISIONS.md`. Create behavior is governed especially by UI-015 through UI-019; initial Library/Media Viewer scope by UI-020/UI-021; persistent uploaded-media identity by UI-022; Library search by UI-023; durable Viewer Download by UI-024; durable display-name Rename by UI-025; and Phase 11 landing/Create routing plus closed-beta launch posture by UI-052. Phase status is tracked in `docs/ui/UI_MIGRATION.md`.

## Penpot workflow
1. Create/open the RenderLab Penpot project.
2. Import the SVG file or drag it onto a Penpot board.
3. Keep each named top-level artboard as a separate reference state.
4. Refine components/layout in Penpot if needed.
5. Record accepted decisions back in the repository before implementation.
6. Verify the real responsive rendered implementation before changing a surface/component to `APPROVED`.

Do not treat an imported SVG as an approved implementation.

## Visual baseline
The files intentionally use the repository design-system baseline:
- canvas `#090A0C`
- surface-1 `#111318`
- surface-2 `#171A20`
- surface-3 `#20242C`
- border `#2B303A`
- text `#F4F5F7`
- text-muted `#9CA3AF`
- accent `#7C6CF2`

Typography is represented with Inter/system sans-serif fallbacks.
