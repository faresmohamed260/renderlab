"""Persisted token rotation state for ComfyUI provider accounts."""
...
def load_tokens() -> list[ModalToken]:
    ...
    if str(os.getenv("RENDERLAB_MODAL_ALLOW_ENV_FALLBACK") or "").strip().lower() not in {"1", "true", "yes"}:
        return []
    payload = str(os.getenv("RENDERLAB_MODAL_TOKENS_JSON") or "").strip()
    if payload:
        try:
...
def update_token_stat(
    ...
    *,
    ...
    if api_url is not None:
        token_stats["api_url"] = str(api_url or "").strip()
    if ui_url is not None:
        token_stats["ui_url"] = str(ui_url or "").strip()
    if health_url is not None:
        token_stats["health_url"] = str(health_url or "").strip()
