...
class ModalComfyUIPoolManager(ModalEndpointPool):
    def __init__(
        ...
        *,
        ...
        load_provider_config: bool = True,
        ...
        if load_provider_config and (not resolved_hf_token or not resolved_app_name):
        ...
        self.studio_mode = bool(studio_mode)
        super().__init__(
            app_name=resolved_app_name or (provider_config.app_name if provider_config else "") or str(os.environ.get("MODAL_COMFYUI_APP_NAME") or "renderlab-image-runtime").strip(),
            tokens=tokens,
    ...
    def execute_studio(self, payload: dict[str, Any]) -> dict[str, Any]:
        return self.execute(studio_payload=payload)

    def studio_status(self) -> dict[str, Any]:
        return self.execute(studio_status_request=True)
    ...
    def _resolve_urls_for_token(self, token: Any) -> dict[str, str]:
        ...
        api_url = urls.studio_api_url if self.studio_mode else urls.api_url
        return {"api_url": api_url, "ui_url": urls.ui_url, "health_url": urls.health_url}
    ...
    def _invoke_endpoint(self, endpoint: dict[str, Any], **kwargs) -> dict[str, Any]:
        ...
        studio_payload = kwargs.pop("studio_payload", None)
        studio_status_request = bool(kwargs.pop("studio_status_request", False))
        if studio_payload is not None:
            payload = client.execute_studio(dict(studio_payload))
        elif studio_status_request:
            payload = client.studio_status()
        else:
        ...
        record_modal_timing(
            ...
            workflow_mode=str((request_metrics.get("workflow_mode") or kwargs.get("workflow_mode") or ("studio" if self.studio_mode else ""))).strip(),
