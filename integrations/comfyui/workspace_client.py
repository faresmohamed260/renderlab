...
def _modal_env(token: ModalToken, *, hf_token: str = "") -> dict[str, str]:
    env = os.environ.copy()
    if token.token_id and token.token_secret:
...
studio_api_url = worker().studio_api.get_web_url()
health = modal.Function.from_name({app_name!r}, "health")
health_url = health.get_web_url()
print(json.dumps({{"ui_url": api_url, "api_url": api_url, "studio_api_url": studio_api_url, "health_url": health_url}}))
"""
    ...
    try:
        ...
        urls = ModalUrls(ui_url=payload["ui_url"], api_url=payload["api_url"], studio_api_url=payload["studio_api_url"], health_url=payload["health_url"])
        record_modal_timing(
            "modal_endpoint_lookup",
            time.perf_counter() - started_at,
            token_name=token.name,
...
def invoke_download_studio_model(
    token: ModalToken,
    app_name: str,
...
import json
import modal
fn = modal.Function.from_name({app_name!r}, "download_studio_model")
payload = json.loads({serialized!r})
print(json.dumps(fn.remote(payload), ensure_ascii=False))
"""
    result = _run([str(PYTHON_EXE), "-c", script], token=token, timeout=timeout)
    if result.returncode != 0:
        raise RuntimeError(result.stderr or result.stdout or "Modal Studio model download failed")
    try:
        return json.loads((result.stdout or "").strip())
    except json.JSONDecodeError as exc:
        raise RuntimeError(f"Failed to parse Modal model download payload: {(result.stdout or '').strip()}") from exc


def invoke_studio_model_catalog(
    token: ModalToken,
    app_name: str,
    *,
    timeout: int = 300,
...
import json
import modal
fn = modal.Function.from_name({app_name!r}, "studio_model_catalog")
print(json.dumps(fn.remote(), ensure_ascii=False))
"""
    result = _run([str(PYTHON_EXE), "-c", script], token=token, timeout=timeout)
    if result.returncode != 0:
        raise RuntimeError(result.stderr or result.stdout or "Modal Studio model catalog failed")
    try:
        return json.loads((result.stdout or "").strip())
    except json.JSONDecodeError as exc:
        raise RuntimeError(f"Failed to parse Modal model catalog payload: {(result.stdout or '').strip()}") from exc
