...
def resend_json(url):
    proc = subprocess.run(
        ["curl", "--fail-with-body", "--silent", "--show-error",
         "--header", f"Authorization: Bearer {RESEND}", url],
        text=True, capture_output=True, timeout=30,
    )
    if proc.returncode != 0:
        raise RuntimeError(f"Resend API request failed: {proc.stderr[:500]}")
    return json.loads(proc.stdout)

domains = resend_json("https://api.resend.com/domains")
...
domain = resend_json(f"https://api.resend.com/domains/{exact[0]['id']}")
