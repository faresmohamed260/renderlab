...
old_status = "**Status: `IN PROGRESS — PHASE 15 COMPLETE / VERIFIED; PHASE 16 CONTRACT ACCEPTED / IMPLEMENTATION NOT STARTED`.**"
new_status = "**Status: `IN PROGRESS — PHASE 16A/16B/16C IMPLEMENTED / VERIFIED; PHASE 16D DESIGN CHECKPOINT PENDING`.**"
if old_status not in text:
    raise SystemExit("Cycle 3 status marker not found")
text = text.replace(old_status, new_status, 1)

old_intro = "Phase 16 planning is now captured by the accepted execution contract below. Implementation has not started, and the contract itself does not authorize production deployment or scheduler activation."
new_intro = "Phase 16 planning is captured by the accepted execution contract below. Implementation is in progress: 16A shared recipe reconstruction, 16B Reuse Settings and 16C successful Run Again are implemented and verified; 16D comparison remains blocked on its required visual design checkpoint. The contract and this implementation evidence do not authorize production deployment or scheduler activation."
if old_intro not in text:
    raise SystemExit("Phase 16 intro marker not found")
text = text.replace(old_intro, new_intro, 1)

old_contract = "# Phase 16 Execution Contract — Creative Iteration\n**Status: `ACCEPTED / IMPLEMENTATION NOT STARTED`.**"
progress = """# Phase 16 Execution Contract — Creative Iteration
