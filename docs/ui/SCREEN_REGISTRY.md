...
## Current production application — 2026-09-21
<!-- RENDERLAB_CURRENT_PRODUCTION_SHA: 986aab269551a4c6e7d3af3a259cfa955c975d91 -->
Current production runs exact repository source `986aab269551a4c6e7d3af3a259cfa955c975d91` at READY Vercel deployment `dpl_D9QyFnD7BqCAB1biqsJRppDqBgnR` (`https://renderlab-8k6imydly-faresmohamed260-6733s-projects.vercel.app`). UI-082 PR #310 merged as that exact source; merged-main Engineering Quality `35547496591`, UI Shell Validation `35547496574`, and Activity Cancel Visual `35547496556` passed. Final cutover `35548015570` explicitly moved `renderlab.faresuniform.uk`, passed custom-domain smoke for root, `/create`, `/library`, `/activity`, `/settings`, `/settings/password`, `/settings/profile`, and `/settings/preferences`, and verified the expected signed-out `/admin` 404 concealment boundary. Vercel metadata reports exact SHA/READY/production/CLI provenance; bounded post-cutover inspection found no runtime errors and no error/fatal logs. The prior deployment `dpl_6Z8LTAE8g91FvT4TaB7NQaX4dBNM` remains the rollback anchor. Automatic Git → Vercel deployment remains disabled.

Whole-product audit `docs/audits/2026-09-17-production-user-audit/REPORT.md` now records QA-001 through QA-005 complete. QA-002 manual run `35440027637` passed the four fixture-owned creative journeys plus public/signed-out and read-only account surfaces; QA-003 remains accepted through run `35374052822`; QA-004 run `35440028603` passed the account/security/data acceptance scope. Final QA-005 run `35453752650` proved bounded provider/reconciliation failures become stable actionable Activity failures with zero real provider spend; artifact `10587349110` contains four human-reviewed desktop/focus/post-refresh/390px-reduced-motion screenshots with verified DB/Auth/R2 cleanup. #278 whole-product production acceptance is complete.
...
**Phase 26 / UI-077 closure — merged / verified:** Activity now renders the three newest visible jobs as the chronological asymmetric `01 / 02 / 03` Job Matrix and continues remaining visible jobs in the attached History Register without changing server ownership, newest-first truth, pagination, lifecycle states or action eligibility. Final exact head `db4d45320d6abd92e729a5fcae9a8f63ce79d3c4` passed all 10 attached workflows; Activity Visual `34755095367` / artifact `10317550573` (`sha256:dbf866939fdb0787a5ea848a1b5c700e1d9bda864b431600ab8c5ecff24b35b6`) was human-reviewed across desktop, 390px, reduced motion, signed-out and retry temporal states. Release Candidate Matrix `34755095388` attempt 2 passed 23/23 exact-SHA children. PR #205 merged as `2a49d7e223595cd2f876829a779d3f5d3c362e81`; all seven merged-main workflows have successful coverage and merged-main Release Candidate Matrix `34756670731` passed its full child matrix. Production remains unchanged pending separate authorization.
...
### Settings
**Route:** `/settings`  
**Status:** APPROVED — UI-078 Trust Register implemented / exact-head verified / fidelity reviewed / merged / merged-main verified / not deployed; existing account/admin security contracts preserved
...
**Phase 27 / UI-078 closure:** `/settings` and `/settings/password` use the bounded Trust Register grammar: state-specific intro copy; read-only Sign-in email; server-derived Access; Change password with truthful session consequences; verified recovery context; global **Sign out everywhere**; and conditional fresh-Admin continuation. Signed-out state remains invitation-only Email/Password plus enumeration-safe recovery. The current 8-character floor, Current/New/Confirm semantics, recovery marker validation, global sign-out behavior, canonical `auth.users.id`, admission/role ownership and Admin gate are unchanged. Profile/avatar/username, reusable Show/Hide/password-policy UX, hosted Auth hardening, session inventory, MFA, email editing, export/deletion, preferences/notifications and passkeys remain out of scope. Final PR head `d48084fef1255ee672936f6da8905ba95bc678ea` passed all six attached workflows; reviewed Account Identity artifact `10320219197` (`sha256:8217b1bfcb314825f17b37dbf3c3e51861a6f61b5e2ac772ccbbda6b21cdccac`) covers desktop/390px signed-out, Active, Suspended, ordinary password and verified-recovery states, while final-head artifact `10321540973` (`sha256:490236398632e5a6c68ee4fca7591da68682b2c13343766f7c6d4903545e62f8`) confirmed the configured flow. PR #227 merged as `383def35fdf77566696a0c41f88349613029a37a`; merged-main Engineering Quality `34769204247` and UI Shell Validation `34769204150` passed. Production remains unchanged.
...
**Approved behavior:**
- compact email/password Sign in + Forgot password when signed out, and access-status / Change password / Sign out everywhere when signed in; public self-service Create account is absent under the Closed-Beta admission contract;
...
**Cycle-start baseline:** `main` `5f1df1c364bf5aca0a47fd964bd3dd21714f22a7` / production `c00664d9d88c09bd4a6794ad1ad5fbe7ce662e8a`.
...
### Activity + Library — UI-082 hierarchy checkpoint
**Status:** COMPLETE / VERIFIED / MERGED / MERGED-MAIN VERIFIED
**Design checkpoint:** `design/rd/activity-library-hierarchy-v01.md`

Activity keeps the UI-077 Job Matrix + History Register, newest-first chronology, lifecycle truth, refresh, Retry/Run Again/Cancel and result-link semantics. UI-082 changes only presentation hierarchy: request summary is no longer heading content, primary/secondary summaries are bounded, and essential job labels are raised toward the 10–12px floor. Library keeps the UI-075 Gallery Rail, media-card grammar, URL/server-owned state, search, source/kind filters, Favorites, Collections, sort, Select and upload behavior. UI-082 reduces only avoidable command-rail visual weight/height and raises essential labels while leaving touch targets and selection choreography intact.

Verification: exact source `f6245c3953d7517d0b40011893135d4a2ac0028f` passed all 18 attached workflows. Activity Visual `35531996901` / artifact `10611591672` (`sha256:4f1fd684ee6518315a93dda67622dc978a8232d6d3eb090469b23cfe8ac05b61`) and Library History Visual `35531996914` / artifact `10611526669` (`sha256:91e24628af5bd7af4d322d7d0a965ca8c5b16f7972367a9b79d4b31cbc12914c`) were human-reviewed at desktop and 390px with reduced-motion/selection states. The deliberately long Activity request remains bounded; media remains Library's primary object; no horizontal overflow or control/action reachability regression was observed. PR #308 squash-merged as `058b8d10b0dcbfc93e47657a2c6ed0cea4c2c7e1`; merged-main Engineering Quality `35543303501`, UI Shell Validation `35543303489`, Activity Cancel Visual `35543303603`, Creative Iteration `35543303491`, and Image Upscale Integration `35543303495` all passed.

### Create — UI-082 checkpoint 4A legibility
**Status:** COMPLETE / VERIFIED ON CHECKPOINT BRANCH
...
### Whole-product — UI-082 checkpoint 5 acceptance
**Status:** COMPLETE / VERIFIED ON EXACT CANDIDATE

Exact candidate `c194ae299f0bdee2243c54eb2be00612c86f03e2` passed all ten PR-attached workflows: Engineering Quality `35545111587`, Brand / Launch Visual `35545111542`, Account Identity Visual `35545111615`, Create Clear Composer Visual `35545111611`, Activity Cancel Visual `35545111592`, Integrated Release `35545111555`, Account/Admin Operations `35545111584`, UI Shell Validation `35545111605`, Create Lifecycle Visual `35545111606`, and Activity Visual `35545111569`. Dedicated Media Viewer Register Fold `35545120954` / artifact `10616312242` (`sha256:026a06d1585537b4beaedaf31d557b9a334420af18a2631f18a516f490a5de89`) passed the long-prompt 1440px, wide-desktop and 390px regression. Release Candidate Matrix `35545344706` passed 23/23 configured child workflows and published manifest artifact `10615743591` (`sha256:699ee1de823e3f0f6355fa3095a2114cb43651bd6614800ef6d55e5e25a68cee`).

Human exact-candidate review covered Brand / Launch `35545111542` / artifact `10616292213` (`sha256:47b2403535695f872bc9ef1dc95dcedf73bd28c2c047c237eb284134557cc342`), UI Shell `35545111605` / artifact `10616302419` (`sha256:4eb2ff9c8c8fbf5f9ab63ce17903cd2038e8773ac75ab89b5d7a7d2a13d06b05`), Create Lifecycle `35545111606` / artifact `10616137801` (`sha256:22323e751a907513aa3a0efe6fb5bc594d026b8a78f24d4737cd429cedcb7b25`), Library History `35545393522` / artifact `10616731894` (`sha256:80f8ede3b749a09db958fc4159c5b993d4c7ed48edaa32d2aab0f6d28c0c854d`), Activity Visual `35545111569` / artifact `10616502062` (`sha256:83423c7e8ce0587005eea8513136410b42331f358eb6d28309b4a2a5c3b87dee`), Account Identity `35545111615` / artifact `10615832262` (`sha256:b31b946e3da82c318f8c6a9df0e224cf90177012bb2c544bf63cab9cc6288b6d`), Account/Admin Operations `35545111584` / artifact `10616442086` (`sha256:cb4fa83fee599346095a56a6efefbb3d8b281fbfbf643861f14bedd479fd0c0f`) and Viewer. Across desktop, 390px and applicable reduced-motion states, each approved route keeps its intended composition while media/current task hierarchy stays primary, prompt text remains bounded, controls/actions remain attached and reachable, and no horizontal overflow or cross-route visual regression appears. No reusable component contract changed; the existing Component Catalog remains current. Production is now live from exact source `986aab269551a4c6e7d3af3a259cfa955c975d91` / READY deployment `dpl_D9QyFnD7BqCAB1biqsJRppDqBgnR`; final documentation synchronization remains the repository closure gate.
