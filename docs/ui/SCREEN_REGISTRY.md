...
**UI-060 corrective extension — Creatives / Uploads:** `/library` now exposes two same-route origin sections over the existing durable asset workspace. Creatives is the canonical default (`origin=generated`); Uploads is `?tab=uploads` (`origin=uploaded`). `All / Images / Videos`, search, Favorites, Collections, sort and bounded pagination remain section-local URL-owned browsing state. Upload and desktop drag/drop belong to Uploads only; newly persisted files therefore remain visible in the section that accepted them. Viewer/deep-link identity and existing media actions remain unchanged. The old v0.1 no-tab restriction is superseded, but the unified durable media-identity rule is retained. No migration or new route is introduced.
