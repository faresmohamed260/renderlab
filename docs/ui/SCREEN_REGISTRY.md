...
## Current production application — 2026-09-18
<!-- RENDERLAB_CURRENT_PRODUCTION_SHA: 2fc64231f8aa0e5a2df8b2698824319a25c4e9f8 -->
Current production runs exact repository source `2fc64231f8aa0e5a2df8b2698824319a25c4e9f8` at READY Vercel deployment `dpl_Cssdq7grVd6eGkN4Y1xqdWPqV8bz` (`https://renderlab-hrydffycn-faresmohamed260-6733s-projects.vercel.app`). Guarded rollout `35373771751` explicitly moved `renderlab.faresuniform.uk`, passed custom-domain smoke for root, `/create`, `/library`, `/activity`, `/settings`, `/settings/password`, `/settings/profile`, and `/settings/preferences`, and did not invoke rollback. Production includes the verified Run Again correction from #285 together with the prior approved application state. Automatic Git → Vercel deployment remains disabled.

Whole-product audit `docs/audits/2026-09-17-production-user-audit/REPORT.md` records QA-001 and QA-003 complete. QA-001 reusable proof run `35413607297` verified the exact current production SHA across the four authoritative current-production records after permanent marker/verifier merge `8028f8be33ef8760c55b7f796ed1a802f0199cd1`. QA-003 manual run `35374052822` passed fixture-safe Activity Cancel/Retry/Run Again plus TOTP enrollment, fresh AAL2 challenge and read-only Admin coverage on desktop/390px. Remaining #278 work is QA-002 manual production execution/evidence review, QA-004 account/security/data production execution, and QA-005 bounded reconciliation/error presentation. QA-002's permanent hardened workflow is merged in PR #298 / `db48d14fa5265036066531cba8aa93d9ac6602f5`; exact-head and merged-main Engineering Quality passed.
