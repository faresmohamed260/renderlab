...
**Status:** PLANNED — UI-058 contract accepted; 18D lifecycle/recovery verified; 18E Viewer action not started
**Route:** existing `/library/[assetId]`; no new Upscale route

**Target behavior:** an eligible active durable image may expose **Upscale 2×** in the existing Continue hierarchy. Starting the action creates a distinct asynchronous owner-scoped `upscale-image` job through a product API while the source Viewer remains stable. Accepted state gives concise local feedback plus Activity continuation; it does not fabricate completion. Source media is never overwritten. A succeeded Upscale result is an ordinary durable image and Phase 18D now verifies UI-056 Compare source while the same-owner source remains active; tombstoning that source removes comparison. The visible `Upscale 2×` source action itself remains 18E.
