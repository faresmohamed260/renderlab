...
**Phase 23 / UI-073 Clear Composer candidate:** `CreateWorkspace` remains the single authoritative Create surface; no parallel workspace or generic composer primitive was introduced. The production candidate promotes the maintained Image/Video ToggleGroup, labelled durable reference/Start-image action, explicit media roles, dominant prompt, conventional essential settings/Advanced/Generate, truthful generation stage and media-first durable result. It keeps existing state/functions for upload, polling, persistence, aliases and continuation, and continues to use Motion for React only for bounded Create-owned layout/media transitions with reduced-motion equivalence. Result-stage styling is scoped to Create in `globals.css`; ordinary controls do not parallax. Final implementation-verification head `e46d9383dc74878713d0ec4f47aaf6383b266c21` passed Engineering Quality `34639132519`, Clear Composer Visual `34639132478`, configured Create Lifecycle `34639132510`, corrected Account/Admin Operations `34639132466` and Release Candidate Matrix `34639132431` with all 23 exact-SHA children successful. No new reusable primitive, architecture boundary or animation runtime was introduced. PR #185 squash-merged to `main` as `d360f60afeca0b6c417ff1c12dec3c7e922c20f7` from final exact head `428cbd025d9f314f5b8d1582fd6911f1abd5094b`, and merged-main Release Candidate Matrix `34645636285` passed all 23 exact-SHA children; production deployment remains separately explicit.

**Phase 23 fidelity correction reopening — historical:** Direct user fidelity review reopened the merged UI-073 pixels without reopening Create product semantics. PR #188 kept `CreateWorkspace` as the single authoritative feature owner and restored the approved v0.5 composition through Create-owned markup/CSS plus the scoped `src/app/create-clear-composer.css`; no generic component, architecture boundary, product capability or animation runtime was added. The correction is now merged as `3f0d21ed55554b3c48791d35dd17cb6005212076`.
...
**Phase 23 / UI-074 fidelity correction — merged:** `CreateWorkspace` remains the single authoritative Create surface with the approved v0.5 900px authoring / 1120px result geometry, mode switch above the composer, reference-before-Prompt grammar, flat labelled settings footer, high-contrast Generate, composer-attached Advanced, truthful registration-framed generating state and asymmetric media-first result rail. Continuation remains capability-derived and existing upload/polling/persistence/alias/native-video/security contracts are unchanged. Accepted browser evidence is artifact `10293387664` (`sha256:e4937a16a2fed2bc160634c73c60322b0748ff3eb5d166b44cbc33f894a92915`) plus lifecycle artifact `10293653778` (`sha256:ba13fc48dcf4d823575b041455d902e48040d2cd554892c0b5b508ee80e88624`). Documentation-final head `b6a2590875432ba75c28db9e0f4b465133e1873c` passed all 16 attached workflows; PR #188 merged as `3f0d21ed55554b3c48791d35dd17cb6005212076` and all five merged-main workflows passed after one unchanged Creative Iteration retry for a transient Supabase 504. Production remains unchanged.
...
**Purpose:** Account-private current/recent generation history with real lifecycle state, sanitized failures, bounded pagination, active-result continuation and server-derived recovery/control. UI-058 extends the existing composition with truthful promptless `2× upscale` summary plus failed Upscale Retry; no new generic component is introduced.
**Dependencies:** `generation-activity-contract`, `generation-activity` server query, `ActivityAutoRefresh`, failed-row `ActivityRetryButton`, native `ActivityCancelButton`, successful prompt-generation `ActivityRunAgainButton`, maintained Alert/Button/Empty, Lucide. Upscale action eligibility remains server-owned; successful Upscale does not gain Run Again. Narrow Activity cards reserve the full card width for job information and move row actions beneath it; a successful `View result` + `Run again` pair shares one mobile action row, while desktop keeps the existing compact right-side actions. PR #114 / merge `14df1b716e1e9d7ba092c898d4aa3a2217ca718b` verified this responsive refinement without introducing a new generic component or changing Activity semantics.
...
**Origin:** RenderLab composition governed by UI-076 / `docs/ui/MEDIA_VIEWER_REGISTER_FOLD_IMPLEMENTATION_CONTRACT.md`, while preserving the approved UI-056 comparison product rules and UI-058 Upscale contract
**Purpose:** Contextual durable-media workspace where the registered media object is the primary workspace, with attached continuation, local disclosures, truthful metadata and durable asset actions.
**Variants:** image/video; generated/uploaded metadata; attached Media Register; Prompt/Details/Manage disclosures; Favorite/Download quick actions; Manage-owned Collections/Rename/Delete; capability-derived continuation; UI-076 Result-primary Source Fold; UI-058 server-eligible fixed 2× image continuation.
**Used by:** `/library/[assetId]`
**Dependencies:** Next.js Link, maintained Button/Alert/AlertDialog/Spinner primitives, Lucide React, Motion for React/CSS for bounded Viewer transitions, `PublicMediaAsset`, shared continuation capabilities, product media/collection/Upscale routes, feature-owned `MediaViewerRegister`, `MediaViewerComparison`, `MediaViewerUpscaleAction`, `MediaViewerActions` and `MediaViewerCollections`.
...
**Notes:** UI-076 Phase 25 is the current composition authority. Exact implementation head `3e468ad0abd4f7b6645e86e298ce53a3a8ecf034` passed the complete acceptance set; dedicated Viewer run `34717466185` produced artifact `10304951419` (`sha256:d551006acffa260e8bbd83e25c2b156204a4391f1f9b1abcfc4e58aa0cc28e04`), reviewed clean across desktop/390px/temporal/reduced-motion states. PR #200 merged as `b672c711f885092c5e92c42824078e7bd5bc691e`, and all six attached merged-main workflows passed. Historical behavior evidence remains: base generated-media Viewer/continuation `33034606396`; Download PR #11; Rename PR #12; Favorites PR #23. UI-032 final head `fa0a6088a2e3fa0c14488b64d7dd6828e7bd6578` passed Collections `33210501106` and the complete affected matrix; PR #24 merged as `143f7bfb0be8b4857e5dd45959466e71ae22a42d` and desktop/mobile Viewer artifacts preserve Continue hierarchy and existing Rename/Download composition. UI-056 exact implementation head `4d1a495a8145238e1e78756c7b09cdbaee8d8115` passed Creative Iteration `33964679539` plus all 26 affected workflows; final comparison artifact `9969057974` was hash-checked and human-reviewed clean with no corrective code change. UI-058 Viewer Upscale implementation head `ac4aed60e64061ee6a911c858cdc032b6f9a7423` passed focused run `34033506667` and preserved that existing comparison/result hierarchy; artifact `9989427506` (`sha256:49e40583899891a1f1863ff8fd49a714febf11bfdb999bb781849f45c3e01121`) was reviewed clean on desktop/narrow states.

### MediaViewerRegister
**Status:** APPROVED
**Source:** `src/features/library/media-viewer-register.tsx`
**Origin:** UI-076 Media Register + Source Fold v0.2 feature-local composition using maintained primitives and existing capability-derived Viewer actions.
**Purpose:** Attach continuation actions and local Prompt/Details/Manage disclosure ownership directly beneath the registered media stage without reintroducing a permanent property sidebar.
**Used by:** `MediaViewer` only.
**Dependencies:** current server-derived continuation capabilities, `MediaViewerActions`, `MediaViewerUpscaleAction`, maintained Button/disclosure mechanics and Viewer-local presentation state.
**Reuse rules:** Keep it Viewer-owned. Preserve server/capability truth, stable stage geometry, keyboard/touch reachability and reduced-motion equivalence; ordinary action behavior stays in the existing feature components/product routes.
**Do not:** Turn the register into global shell chrome, a generic property inspector, a client capability registry, a second media state store or a place to expose provider/worker/storage identity.
**Notes:** Approved under UI-076. Exact-head Viewer evidence `34717466185` / artifact `10304951419` verified default/disclosure states, Source Fold continuity, narrow composition and reduced motion; PR #200 merged as `b672c711f885092c5e92c42824078e7bd5bc691e`.
...
**Origin:** RenderLab feature-local comparison semantics established by UI-056 and current Source Fold presentation governed by UI-076.
**Purpose:** Progressive Result-primary Source Fold for an eligible generated result, preserving truthful source/result media and keeping Source contextual rather than creating a second management surface.
**Used by:** `MediaViewer` only.
...
**Reuse rules:** Keep comparison eligibility server-derived and owner/current-state validated. Desktop reallocates the same registered stage with Result materially primary; narrow layout keeps full-width Result first and stacks Source below. Source exposes only `Open source`; result Prompt/Details/continuation/actions and native video controls remain result-owned. Reduced motion renders the complete settled relationship without spatial transforms.
...
**Notes:** UI-056 exact implementation head `4d1a495a8145238e1e78756c7b09cdbaee8d8115` established eligibility/ownership semantics. UI-076 exact implementation head `3e468ad0abd4f7b6645e86e298ce53a3a8ecf034` supersedes the old comparison presentation with Source Fold; Viewer artifact `10304951419` verifies t000/t060/t180/t360 progression, narrow Result→Source stacking and reduced motion. No new route, durable comparison state or generic primitive was introduced.
...
**Purpose:** Viewer-owned durable action composition under UI-076: Favorite and Download are singular quick actions; Collections, Rename and UI-033 permanent Delete are owned by the local Manage disclosure, with existing mutation/confirmation semantics preserved.
**Used by:** `MediaViewer` only.
...
**Notes:** UI-076 supersedes the old vertical action-stack presentation: Favorite/Download are quick actions and Collections/Rename/Delete live in Manage; product semantics and server routes are unchanged. UI-033 destructive confirmation remains the maintained AlertDialog contract. Final exact head `53b0eb4c648b47a17fee2e735b7dddc85d345518` passed all 15 affected gates and desktop/mobile confirmation review; PR #25 merged as `40945ff8c4c7e3a3db0e115c4d7cae9f50db4445` and merged-`main` checks remained green.
