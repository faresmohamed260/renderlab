...
- UI-034/UI-049 Library batch selection is transient current-page browser state, not URL/durable state. Library navigation resets selection. Batch Delete composes the existing owner-scoped UI-033 mutation per item; Phase 8B adds explicit Favorite/Unfavorite and Add/Remove Collection target-state batch APIs. All remain bounded best-effort per item rather than claiming cross-service transactionality, and active server-owned filters prune moved-out items after refresh.
...
PR #73 merged the verified UI-052 routing boundary shown above to `main` as `46c5daa2866c6758907ee9be219bcb3cb274ca83`:
...
### Phase 10 account/admin/admission boundary — 10A–10D implemented / verified
**Phase 10C verified state (2026-08-30):** exact code/test head `ca8e426066385934b296b6d4f88324e9c12861f7` passed all 22 affected workflows. `/admin` and `/api/admin/**` retain fresh `auth.getUser()` + active-admin authorization; RenderLab-only account discovery and invitation anti-enumeration remain unchanged. `0012` is applied as `20260830101734 renderlab_generation_admission`. Per-account overrides and typed singleton global defaults now feed one transactional `submitGeneration` admission boundary shared by Create and Retry before native/external backend dispatch. 10D hardening and deployment remain outside this slice.
...
Implemented Phase 10B/10C privileged APIs are narrow and server-authorized:
```text
...
```
They expose only RenderLab access/invitation/account-override state, the three typed global generation defaults, and sanitized aggregate product health. No raw `auth.users` directory, passwords, provider credentials/routing, arbitrary feature flags or account deletion. `/api/admin/settings` requires the same fresh active-admin boundary and atomically writes all three bounded values.
...
Implemented server-owned Phase 10 records are `renderlab_account_access`, `renderlab_beta_invitations`, `renderlab_beta_settings`, and `generation_admission_reservations`. RLS is enabled with browser grants revoked. Invitation/admin/admission/settings privileged routines use explicit empty search paths with execute revoked from public/anon/authenticated and granted only to service-role/server execution. No generic client global account/admin store is introduced.
...
**Phase 10D merged-main acceptance:** PR #70 merged as `5950958dc58143b099bc2877a942829c045f700e`. Reference Upload `33314652092`, UI Shell `33314652107`, Generation Integration `33314652101`, and Video Generation Integration `33314652229` all passed on the merge commit; exact deterministic push-run fixture cleanup returned Auth/access/reservations/jobs/sources/media/uploads to zero and preserved the enabled / 1 / 12 / no-updater singleton. Phase 11 requires its own merged execution-ready contract before implementation.
...
POST     /api/media/assets/batch-favorite
...
PATCH    /api/media/collections/[collectionId]
DELETE   /api/media/collections/[collectionId]
POST     /api/media/collections/[collectionId]/items/batch
...
`GET /api/media/assets` accepts bounded `kind`, `q`, `sort`, `favorite`, `collection`, `limit`, `offset`; `favorite` accepts only `true`, `collection` requires a UUID when present, and `sort` accepts only `newest|oldest` with newest as default. `PATCH /api/media/assets/[assetId]` remains the UI-025 Rename mutation; UI-031 uses idempotent owner-scoped favorite PUT/DELETE; UI-032 uses owner-scoped collection list/create plus idempotent membership PUT/DELETE. UI-049 Phase 8A adds owner-scoped collection `PATCH` rename and `DELETE` lifecycle mutations; rename reuses the existing normalized 1–120 owner-unique naming contract, and delete removes only the owner collection/cascade memberships while preserving media/Favorites/R2/history. UI-049 Phase 8B adds owner-scoped `POST /api/media/assets/batch-favorite` and `POST /api/media/collections/[collectionId]/items/batch`: both validate a raw 1–24 UUID array before mutation, dedupe for processing/summary, use explicit target-state booleans, return per-item best-effort outcomes, collapse foreign/missing/tombstoned assets to `asset_not_found`, and prevalidate collection identity once before asset processing. UI-033 adds owner-scoped idempotent `DELETE /api/media/assets/[assetId]`: tombstone first, then R2 primary/thumbnail purge, with `purged_at` recorded only after physical cleanup succeeds. UI-034 adds owner-scoped `POST /api/media/assets/batch-delete`, capped at 24 deduplicated UUIDs, which composes that same single-asset deletion per item and returns per-item outcomes/summary. Picker and drag/drop persistent uploads both use the same existing media-upload ticket/completion APIs. Browser components do not call workers, Supabase service-role APIs or raw R2 credentials directly.

UI-050 Retry uses the same verified account boundary. The owner-scoped route accepts only the historical UUID, reconstructs stored product intent server-side, reuses current request/input validation and ordinary submission, returns product-level retry errors, and never exposes or forwards historical workflow/model/worker/provider/failover/error/output execution metadata. No Phase 9 schema migration was added.
...
│       ├── library-collection-menu.tsx
│       ├── library-collection-manager.tsx
...
- `server/media` — owner-scoped durable media/query/upload/download/rename/delete/reference plus Collections list/create/membership, UI-049 collection rename/delete lifecycle services and reusable owner-scoped Favorite/collection-membership mutations composed by the bounded Phase 8B batch routes;
...
Use for Create workspace/polling, temporary reference interaction, Library upload file selection/feedback, Library transient drag/drop interaction, UI-034/UI-049 page-scoped selection/batch-delete/organization feedback, the small Library sort/collection navigation menus, UI-049 collection-manager disclosure/form/confirmation state, Viewer Favorite/Collections/Rename interaction state plus UI-033 Delete confirmation/busy/error state, Activity's small refresh timer while work is active, Settings account form actions and interactions that truly require browser state.
...
### Phase 16 successful-history + Viewer comparison boundary — COMPLETE / VERIFIED
...
### Phase 18 Image Upscale frontend/domain boundary — UI-058 — COMPLETE / VERIFIED through 18F
Phase 18 adds no top-level route and no new Create mode. The visible entry point is **Upscale 2×** inside the existing Media Viewer Continue hierarchy for a server-eligible active durable image. The Server Component loads the current owner-scoped asset, derives conservative eligibility from durable kind/MIME/size/geometry plus configured Upscale backend presence, and passes only a boolean into the Viewer. The browser submits only the current opaque asset route identity through `POST /api/media/assets/[assetId]/upscale`; arbitrary scale/model/workflow/source/storage/provider identity never enters client state.

`MediaViewerUpscaleAction` is intentionally feature-local. It owns only a synchronous duplicate-submit lock plus `idle → submitting → accepted|error` presentation using the maintained Button, Spinner and Alert layer. Starting keeps the same full-width secondary Continue row and shows `Starting upscale…`; accepted work leaves the source Viewer stable, keeps the action locally locked as `Upscale started`, and offers **Open Activity** for asynchronous truth. Rejection restores a retryable action with sanitized local feedback. There is no Viewer job polling, global client store, result placeholder, fake percentage or ETA.

The existing prompt-centric Create boundary remains unchanged. `GenerationRequest` still resolves only the four prompt operations; Upscale uses the separate fixed `UpscaleImageCommand`. Phase 18C owns source-byte/MIME/decoded-geometry preflight, generation admission and native fixed-2× dispatch. Phase 18D owns native polling/reconciliation/finalization/cancellation, failed Retry, truthful Activity `2× upscale`, and conditional succeeded-result Compare source. Successful Upscale Run Again/Reuse Settings remain unavailable.

The required repository-backed desktop + 390px Viewer checkpoint is recorded in `design/penpot/media-viewer-v0.3-upscale-2x.md` and its two SVGs. Implementation head `ac4aed60e64061ee6a911c858cdc032b6f9a7423` passed Upscale Viewer Visual `34033506667`; artifact `9989427506` (`sha256:49e40583899891a1f1863ff8fd49a714febf11bfdb999bb781849f45c3e01121`) was independently hash-checked and human-reviewed clean across eligible, starting, accepted and error states. The 2+1 Continue hierarchy preserves Edit primary + Animate secondary with Upscale as a full-width secondary row; narrow layouts do not horizontally overflow; UI-056 comparison ownership remains unchanged. No new generic primitive, route, schema, production deployment, worker redeploy or scheduler activation was introduced by 18E. Phase 18F completed as a verification-only slice and did not alter this frontend composition.

Image Upscale Integration `34042759032` on exact proof head `fec2ad2762d21cbd92c8db03c8a95130a6b6ecb7` verified the same browser/server boundary end-to-end. Artifact `9992236657` (`sha256:68e5755c3c32b64e7f6606a7c7c94a8ed0774871975c3cf611759b46dee4eaf9`) includes desktop + 390px succeeded-result comparison renders; human review confirmed Result primary, contextual Source with only `Open source`, Edit / Animate / full-width `Upscale 2×`, secondary `Close comparison`, result-owned Actions and no narrow horizontal overflow. 18F adds no route, client store, component, design-system rule or production runtime coupling.
...
- Library cards prefer durable thumbnail URLs. New durable image uploads and generated image outputs persist a best-effort WebP preview bounded to a 640px maximum edge; the original media remains authoritative and full-content fallback remains required if preview creation fails. Legacy active image rows are repaired by an explicit bounded maintenance backfill rather than by transforming originals during Library requests.

### Post-Cycle 3 contextual model-intent boundary
Create now carries a stable model choice as part of normalized prompt-generation intent. `GenerationRequest.model` accepts product IDs rather than worker/provider identity. The parser validates model/output compatibility and supplies backward-compatible defaults for historical requests. Native generation maps that product model ID to the existing server-owned workflow/ecosystem and persists it inside `generation_jobs.parameters`, so recipe reconstruction, Run Again and Retry retain the choice without replaying historical worker/provider state.

UI ownership stays inside the existing Create composition: Image mode uses the maintained Button + Radix DropdownMenu mechanics to choose **FLUX.2 Klein** or **Qwen Image Edit**; Video omits a model control while only REDGraft is productized. `CreateAdvancedPanel` receives only the selected image model needed to render truthful model-specific fields. Qwen's fixed 4-step / CFG 1.0 path hides FLUX-only Steps/Guidance; server parsing and the native adapter enforce the same constraint independently. There is no global model store, Models route, new primitive, schema migration or browser-visible worker/failover contract.

`RENDERLAB_TEST_NATIVE_WORKER_OVERRIDE=true` plus `RENDERLAB_TEST_NATIVE_WORKER_GATEWAY_URL` is a test-only native submission seam used by the Image Model Routing workflow so routing can be verified against a local mock without provider spend. Production behavior ignores that seam unless the explicit test flag is set and continues to use `worker-fleet.ts` registrations.

The contextual Image model-intent implementation was accepted on exact head `d5f80c02f041b645b7a3b1dee13d70d8139cc832` after all 19 attached workflows passed; PR #115 merged as `e8649ba39e660e2ec98cfd2fd864b22db74e64e3`. This changed no route, schema, worker deployment or production runtime state.

### Post-Cycle 3 Library origin-section boundary — UI-060
The Library remains one `/library` route and one durable `media_assets` domain. Server-rendered page state maps the user-facing section to the existing indexed origin field: canonical Creatives → `generated`, explicit `tab=uploads` → `uploaded`. `listMediaAssets` owns the optional server-side origin constraint so filtering/pagination happen before serialization rather than as a client-only card filter. `GET /api/media/assets` exposes the same origin constraint only when requested and preserves its prior unified behavior when omitted.

The browser does not create a second upload store. `LibraryView` carries the section through kind/search/Favorites/Collections/sort/pagination URLs, keys page-scoped selection by section, and enables `LibraryUploadButton` / `LibraryDropUploadSurface` only for Uploads. Viewer routes and asset-level actions remain origin-agnostic because durable asset IDs are unchanged. Migration `0003_persistent_media_uploads.sql` already supplies the `origin` constraint and `media_assets_origin_created_at_idx`; UI-060 adds no schema or infrastructure change. Exact reconciled head `a81c02e82abb0cec6b386e4cfb69f1075f377e33` passed all 30 attached workflows and merged as `999d32812e74afdba73e3cbaa7607a64ce65d603`.
...
## Account data lifecycle boundaries — #219

The verified #219 implementation keeps account-data lifecycle orchestration server-owned. Settings renders the feature-local Data & Privacy surface, while `/api/account/data-export`, `/api/account/data-export/download`, and `/api/account/delete` enforce owner authorization and delegate durable work to `src/server/account/account-data-lifecycle.ts`. Destructive deletion reuses fresh password proof plus recent TOTP/AAL2 step-up when MFA exists; UI state never substitutes for those server checks.

Export status is durable and browser-independent. Durable media is represented by authenticated manifest/download routes rather than by exposing R2 keys or promising one unbounded ZIP. Account deletion freezes new product work before quiescence and removes Supabase Auth only after storage proof and transactional product cleanup succeed. Exact head `886a4722826b268ad156eda22e817002471cd04c` passed the configured lifecycle, Session Controls, Account Identity, Account/Admin Operations, UI and regression gates. Production deployment remains separate.
