...
### Phase 18 Image Upscale frontend/domain boundary — UI-058 — 18D lifecycle/recovery verified, Viewer action pending
...
Phase 18C preserves the prompt-centric Create boundary while adding the server-owned continuation seam. `GenerationRequest` still resolves only the four prompt operations; Upscale uses the separate fixed `UpscaleImageCommand` and owner-scoped `POST /api/media/assets/[assetId]/upscale`. The browser sends no scale/model/workflow/source payload beyond the route asset ID. The server reloads the active same-owner durable image, validates stored bytes/MIME/decoded geometry, reserves ordinary generation admission and submits the native `renderlab-upscale-01` worker with fixed scale 2. Persisted/public history remains null-safe for prompt and recipe reconstruction still rejects Upscale. Phase 18D keeps the client boundary unchanged while completing the server/product lifecycle seam: native Upscale polling/reconciliation/finalization and cancellation are first-class; failed Retry reconstructs canonical fixed-2× intent and current-revalidates its durable source; Activity renders `2× upscale`; and the existing Viewer comparison loader conditionally resolves succeeded Upscale source media only while that same-owner source remains active. Successful Upscale Run Again/Reuse Settings remain unavailable. The Viewer eligibility/action client mutation itself is not implemented yet and remains the 18E design/implementation gate.
