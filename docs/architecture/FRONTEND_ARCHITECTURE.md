...
- Library `tab`, `kind`, `q`, `sort`, `favorite`, `collection`, `offset` are URL-owned shareable browsing/discovery/organization state after account context is resolved. UI-060 makes Creatives the canonical default (omitted `tab`, server scope `origin=generated`) and Uploads the explicit `tab=uploads` section (`origin=uploaded`); changing section clears stale pagination while preserving compatible kind/search/Favorites/Collections/sort state. `favorite=true` remains the UI-031 Favorites filter; UI-032 adds optional `collection=<uuid>` without changing route hierarchy. UI-049 Phase 8A keeps collection lifecycle management on the same `/library` surface; deleting the active collection removes only `collection` plus stale `offset` while preserving compatible filters.
