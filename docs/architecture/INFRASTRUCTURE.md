...
- QA-003 is therefore complete without a reproduced P0–P3 defect. Issue #278 remains open because QA-001/QA-002/QA-004/QA-005 are separate roadmap items. QA-004 is unblocked but was not started by this operation. Automatic Git → Vercel deployment remains disabled; this rollout changed application deployment state only and introduced no Supabase schema/Auth policy, R2 resource-contract, worker/provider/routing or scheduler mutation.

## QA-004 production account/security/data audit harness — 2026-09-18
- Execution authority is `docs/audits/QA_004_ACCOUNT_SECURITY_DATA_PRODUCTION_AUDIT_CONTRACT.md`. The permanent audit harness merged through PR #292 as repository source `4254f07ab044d2d1d815498844513bf57e451ba6`; exact PR head `559def62a8336c95f8aa7fdbf7adba41fb100ef1` and merged-main Engineering Quality run `35397924862` passed.
- The workflow is `.github/workflows/production-qa004-account-security-data.yml` and is `workflow_dispatch` only with `cancel-in-progress: false`, exact 40-character expected-production-SHA input, independent-source-verification acknowledgement and explicit fixture-only destructive-work acknowledgement. Pull requests and pushes cannot execute the production audit.
- The orchestrator `scripts/verify-production-qa004-account-security-data.mjs` reuses the proven profile/credential, preferences/export/delete and session-control verifiers against `https://renderlab.faresuniform.uk`. It does not invoke the configured #219 account-data-lifecycle suite, does not dispatch provider-backed generation and records email change as skipped unless its separate owned-mailbox/rate-limit gate is met.
- Cleanup is unconditional. The orchestrator cleans the deterministic configured profile/preferences fixtures, removes current-run session-control fixture identities, then independently verifies the known QA-004 Auth/database/R2 residue boundary. Evidence is uploaded under one QA-004 artifact root with a manifest.
- Initial PR authoring exposed and corrected a workflow-definition serialization defect where the YAML had been emitted as one line containing literal `\\n` sequences. The unit guard now requires real multiline YAML and rejects literal newline serialization; the corrected workflow is the only merged definition.
- Production application state is unchanged by the harness merge. The live source remains `2fc64231f8aa0e5a2df8b2698824319a25c4e9f8` at READY deployment `dpl_Cssdq7grVd6eGkN4Y1xqdWPqV8bz`; QA-004 production execution has not yet occurred and no deployment/Auth-policy/R2-resource/provider/worker mutation accompanied the harness merge.
