...
- User audit `docs/audits/2026-09-17-production-user-audit/REPORT.md` is blocked by P2 issue #279. Corrective run `35269965598` accepted a live Image job, then observed Activity remain `RUNNING` for 30 minutes without exposing View result; the owner independently confirmed media completes while the site fails to refresh. Run-owned Auth/job/media/R2 cleanup passed. Suspected ownership spans Activity auto-refresh and native poll/reconciliation/finalization; root cause is not yet established. No infrastructure mutation, hosted Auth mutation, fix or deployment followed.
