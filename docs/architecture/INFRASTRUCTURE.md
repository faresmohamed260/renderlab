...
- This maintenance changed only existing private R2 thumbnail objects and the existing nullable `media_assets.thumbnail_storage_key` values. It did not create a bucket/resource, change schema, deploy the application, redeploy a worker or activate a scheduler. Vercel production remains `dpl_6htPrpLMysfqZycZ7wQ5btwejXPA` on application source `c493eaead6997cce1c22c6835c98177d6346ff41`.
