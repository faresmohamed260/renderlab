...
- Production run `35440027637` from harness head `53e847b2ffe2517688de7aa621c5fbaf3f1e78a0` passed against exact live source `2fc64231f8aa0e5a2df8b2698824319a25c4e9f8`. Exactly four fixture-owned generations completed, and public/signed-out plus read-only Settings/Profile/Preferences/session coverage passed.
- Artifact `10583333354` has digest `sha256:4e6208f8a223fb68c8eccc6a327fdb65e377075222729da60beb2128fad0b227`; its manifest records four provider generations, cleanup `verified=true`, nine tracked R2 objects checked, zero contracted DB/Auth residue and 38 evidence screenshots. Human review accepted desktop/390px/focus/reduced-motion evidence.
- Pre-clean, in-run cleanup and unconditional post-clean all passed. The audit changed no deployment, alias/environment, Supabase schema/Auth policy, R2 resource contract, worker/provider configuration or routing.
...
- QA-003 artifact `10559613086` has digest `sha256:fcba296a446a28c0867654018ec4692c2af5c2779902902a6774f3020f18e92c` and contains 32 screenshots plus `manifest.json`. Human review accepted desktop, 390px, visible-focus and reduced-motion evidence without a horizontal-overflow blocker or sensitive-value leak. Exact pre- and post-audit DB/Auth fixture absence was verified; the manifest records cleanup `verified=true` with four R2 objects checked.
- QA-003 is therefore complete without a reproduced P0–P3 defect. QA-002 and QA-004 subsequently completed on the same production application source; at that closure point, issue #278 remained open only for QA-005. QA-005 is now complete as recorded below. Automatic Git → Vercel deployment remains disabled; the QA-003 rollout changed application deployment state only and introduced no Supabase schema/Auth policy, R2 resource-contract, worker/provider/routing or scheduler mutation.

## QA-004 production account/security/data audit — 2026-09-19
- Execution authority remains `docs/audits/QA_004_ACCOUNT_SECURITY_DATA_PRODUCTION_AUDIT_CONTRACT.md`; permanent harness PR #292 merged as `4254f07ab044d2d1d815498844513bf57e451ba6` and merged-main Engineering Quality `35397924862` passed.
- Manual production run `35440028603` from harness head `53e847b2ffe2517688de7aa621c5fbaf3f1e78a0` passed against independently reverified source `2fc64231f8aa0e5a2df8b2698824319a25c4e9f8`. It exercised run-owned profile/avatar, preferences, session controls, password/recovery presentation, MFA assurance, data export and fixture-only account deletion.
- No provider-backed generation was dispatched. Conditional email change was recorded as `skipped_gate_not_met`, preserving the dedicated-mailbox/rate-limit boundary.
- Artifact `10584065519` has digest `sha256:36af1273b690c1bffe35b50a4bbd77752a4df638d7406d89f3e2e55ca914b38f`; the manifest records cleanup `verified=true`, seven configured accounts checked, eight known R2 objects checked, session-run users checked and 11 evidence screenshots. Human review accepted the secret-safe desktop/390px/reduced-motion evidence.
...
Do not reapply migrations 0003 through 0023; all are recorded above as applied to the approved shared project. The required 0005 sequencing was satisfied: exact owner-aware application SHA `5f5d3cee9b45af175f072050f48da4549d5f416c` became READY in production, live account isolation passed, and a final zero-unowned-row audit completed before enforcement. UI-031 implementation-head verification left all four core tables empty, zero RenderLab fixture Auth users, zero browser core-table grants, four RLS-enabled core tables, four `NOT NULL` owner columns and all six UI-030 enforcement triggers intact; `favorited_at` remains nullable and `media_assets_owner_favorite_created_at_idx` remains present.
...
- #286 hardens that cleanup contract after QA-003 found two historical fixture identities from 2026-09-12. The shared configured-account deletion helper now retries idempotent owner-row/access/Auth cleanup and proves exact DB/Auth absence. Activity Cancel no longer suppresses cleanup errors and has an unconditional cleanup workflow leg; Library lifecycle retries its exact idempotent cleanup sequence so transient shared-project 5xx/504 responses fail only after bounded retries. Fixture ownership and `cancel-in-progress: false` safety boundaries are unchanged.
...
Landing production rollout on 2026-09-11 established an additional operational requirement for the current project configuration: a successful CLI `vercel deploy --prod` can create a READY production-target deployment and move the stable project Vercel alias without moving the custom domain `renderlab.faresuniform.uk`. After deployment `dpl_3Smw21pn4PPQ1DzN7aaWmTuuf991` became READY from exact source `0173c4c5ba08360b6352331118abc81978cfa774`, the custom domain still served the prior deployment until explicit `vercel alias set <deployment-url> renderlab.faresuniform.uk` run `34547608773`; that alias operation then passed root + application-route smoke. Future guarded production rollouts must therefore verify the custom domain independently and explicitly assign it when required. For rollback on the current plan, prefer explicit alias restoration to the last known-good deployment rather than assuming deep `vercel rollback <old-url>` is available; the latter returned a plan-limit 402 during rollout run `34547306794`.
...
The Phase 18 work branch records the repository-side prerequisite and the subsequently verified live worker state:
...
**Live 18A evidence:** with explicit user authorization limited to the new Upscale worker, exact RenderLab source `3b7f4a4dcc27a64e1423cbd2c6d0993b24ceb3e8` was deployed as Modal app `renderlab-image-upscale` in verified healthy workspace `modal-45`, worker identity `renderlab-upscale-01`. The first live POST exposed a FastAPI annotation-scope defect (`UploadFile` was route-local while postponed annotations required global resolution); the source and offline verifier were corrected before redeployment. Corrected smoke passed invalid-scale `400` plus a real 8×8 → 16×16 PNG lifecycle. Corrected full proof run `34000980137` passed all 10 accepted cases, including exact RGB/alpha/EXIF geometry, animated rejection, cancellation with no result, and 4096×1024 → 8192×2048 PNG transport. Artifact `9979491468` ZIP digest is `sha256:4a8f2a06b0bcf305eaa9b270bbfbe5937e0f8b5abeb004f5ab57c2665e7e6d04`; `evidence.json` is `sha256:0ad85f892357f1e6d611b32b27a03fa030eef8df3ffe7afb3dae2950919a1dab`; persistent Supabase/R2 fixtures are both zero. Phase 18A is therefore complete/live-verified. No production RenderLab application deployment, scheduler activation, product routing, `pg_cron` or `pg_net` activation followed this worker deployment. Phase 18B has since completed the separately audited domain/schema slice recorded below.

#### Phase 18B shared schema/domain — COMPLETE / VERIFIED
A fresh pre-DDL audit on 2026-09-06 found the live `generation_jobs` operation check still limited to Create/Edit/Video/Animate, `prompt` column-level `NOT NULL`, no null/blank historical prompts in the audited rows, RLS enabled, zero direct browser grants and zero same-owner media/job link violations. The migration was therefore kept deliberately additive/compatibility-oriented rather than rewriting historical intent.

Repository `0018_image_upscale_job_semantics.sql` is applied to shared Supabase project `rashyleshocuvpgcooxy` as `20260906004810 renderlab_image_upscale_job_semantics`. It extends `generation_jobs_operation_check` with `upscale-image`, drops the column-level prompt `NOT NULL`, and adds `generation_jobs_prompt_semantics_check`: Upscale requires `prompt IS NULL`; the four prompt-generation operations require non-null/nonblank prompt. It adds no table/function/RPC/policy/grant, does not touch `media_assets` output-slot identity, and does not encode source ownership inside JSON. Same-owner active image eligibility remains an 18C server reload responsibility.

Post-DDL audit verified zero invalid rows, RLS still enabled, zero `anon`/`authenticated` `generation_jobs` grants and zero generated-media/job owner-link violations. The Security Advisor produced no new Phase 18B-specific finding; the existing intentional RLS/no-policy INFO and unrelated leaked-password-protection warning remain tracked. At 18B closure the shared schema was deliberately ahead of the product route in a rolling-compatible way so existing deployed prompt-generation code could continue writing nonblank prompt jobs; 18C subsequently added the owner-scoped `upscale-image` product route/admission/native-dispatch path. No Vercel rollout, scheduler activation, `pg_cron`, `pg_net` or worker redeployment accompanied the 18B migration.


#### Phase 18C product API/admission — COMPLETE / VERIFIED
The RenderLab application now has a server-only native Upscale submission boundary without changing the shared schema. `POST /api/media/assets/[assetId]/upscale` accepts no browser settings/body, resolves the current account, reloads only an active same-owner durable image and validates R2 content identity plus decoded PNG/JPEG/WebP bytes/geometry before reserving generation capacity. Foreign/deleted/missing sources stay behind the existing owner-scoped not-found boundary.

Accepted work persists the canonical `upscale-image` / `prompt=NULL` / scale-2 intent and uses the existing `renderlab_reserve_generation_admission` → native dispatch → `renderlab_bind_generation_admission` flow. The native worker URL comes only from server environment `RENDERLAB_UPSCALE_WORKER_GATEWAY_URL`; the future Vercel environment preflight now requires that variable and HTTPS, but no gateway value is committed and no production RenderLab environment/deployment was changed by 18C. The optional external prompt-generation backend remains unchanged and does not receive Upscale work.

Configured Generation Integration `34004300165` passed same-owner/foreign/payload-boundary/native multipart/admission/persisted-intent checks with exact fixture cleanup; Generation Admission `34004300161`, Engineering Quality `34004300102` and Deployment Readiness `34004300196` also passed. No 18C database migration, worker redeployment, production application rollout, scheduler activation, `pg_cron` or `pg_net` activation occurred. Phase 18D subsequently verified server-owned Upscale polling/reconciliation/finalization, native cancellation, failed Retry with current-source revalidation, truthful Activity summary and conditional succeeded-result Compare-source recovery. The configured Reconciliation/Cancellation suites use a run-owned test Upscale worker override only; production Upscale worker configuration/deployment is unchanged. The visible Viewer `Upscale 2×` action is implemented/render-verified in Phase 18E at `ac4aed60e64061ee6a911c858cdc032b6f9a7423`; this UI slice changed no worker, environment, schema, scheduler or production deployment state. Phase 18F subsequently completed the dedicated product/live proof. No 18D/18E/18F database migration, production application rollout, scheduler activation, `pg_cron`, `pg_net` or worker redeployment occurred.

#### Phase 18E Viewer exposure — infrastructure unchanged
Viewer implementation head `ac4aed60e64061ee6a911c858cdc032b6f9a7423` passed Upscale Viewer Visual `34033506667` with artifact `9989427506` (`sha256:49e40583899891a1f1863ff8fd49a714febf11bfdb999bb781849f45c3e01121`) plus the implementation-head generation/lifecycle regressions. 18E adds only server-derived Viewer eligibility and a feature-local payloadless browser mutation over the already-verified 18C route; it does not alter the SwinIR worker deployment, `RENDERLAB_UPSCALE_WORKER_GATEWAY_URL` contract, shared Supabase schema/migrations, R2 layout, generation admission, production Vercel environment, scheduler, `pg_cron` or `pg_net`. Phase 18F subsequently completed the separate product/live proof without changing those runtime boundaries.

#### Phase 18F dedicated product/live proof — COMPLETE / VERIFIED
Exact proof head `fec2ad2762d21cbd92c8db03c8a95130a6b6ecb7` passed Image Upscale Integration `34042759032`. The live leg used the already-deployed worker and persisted a real slot-0 PNG at `8×6 → 16×12`, preserving the source; mock/live configured cleanup passed. Artifact `9992236657` independently matches GitHub at `sha256:68e5755c3c32b64e7f6606a7c7c94a8ed0774871975c3cf611759b46dee4eaf9`.

The permanent CI workflow prefers `secrets.RENDERLAB_UPSCALE_WORKER_GATEWAY_URL`; because RenderLab Actions currently has neither that secret nor Modal credentials, its dedicated proof falls back to the independently provider-verified public endpoint `https://silentspecter67--renderlab-image-upscale-web.modal.run`. This is **CI proof-only**, not production runtime configuration: production still requires server-only `RENDERLAB_UPSCALE_WORKER_GATEWAY_URL`, and no Vercel environment was changed. Phase 18F performed no worker redeployment, schema migration, provider-routing mutation, Vercel deployment, scheduler, `pg_cron` or `pg_net` activation. Exact proof head closed at 32/32 attached workflows successful; PR #111 was subsequently squash-merged as `b8be87453ba0f98e3cd70a3c16a6ad9c1747b75d` from `c40705d760639eb6fffdf9d51fc69ed397fc55a0`. Merged `main` passed all 14 workflows / 15 checks GitHub attached, including the bounded real-worker Image Upscale leg. Production RenderLab deployment/environment and scheduler state remained unchanged.

## Cycle 3 / Phase 18 Production Application Rollout — 2026-09-06
**Status: `COMPLETE / VERIFIED / LIVE`.**

- Exact application source `c493eaead6997cce1c22c6835c98177d6346ff41` was explicitly deployed via Vercel CLI as production deployment `dpl_6htPrpLMysfqZycZ7wQ5btwejXPA` (`https://renderlab-42skee4j0-faresmohamed260-6733s-projects.vercel.app`); `https://renderlab.faresuniform.uk` is an active alias. The prior READY deployment `dpl_CZZvmdN42VHRK7uLVUA9W8kdc7x2` was retained as rollback until smoke completed and was not needed. Automatic Git → Vercel deployment remains disabled.
- Production environment-key metadata contained the existing Supabase/R2 contract and server-only `RENDERLAB_UPSCALE_WORKER_GATEWAY_URL`; the deployment's own `prebuild` / `scripts/verify-vercel-env.mjs` validated the actual injected values and shared Supabase identity before Next.js compiled. The Upscale gateway variable already existed and was preserved. The already-proved Modal Upscale worker passed health; no worker redeployment occurred.
- Rollout run `34056745193` verified the custom Closed-Beta domain, exact R2 upload CORS, durable Create Image, Run Again admission, failed Retry history immutability, native cancellation, production Viewer `Upscale 2×` and real `8×6 → 16×12` persisted Upscale with source bytes unchanged. Exact run-owned Auth/access/job/source/media/upload/admission-reservation/R2 fixtures returned to zero.
- Vercel error/fatal runtime-log audit for `dpl_6htPrpLMysfqZycZ7wQ5btwejXPA` returned no entries during the rollout window. Evidence artifact `9996244698` has JSON `sha256:490d14dbe13f6b018deccc28efde78a9fab959ab085138795712ac06b2f4a1a6` and artifact ZIP `sha256:c9e3d8829f4605b8fcb93cc8c4e44b1ca8bae5852934aef88384fd17292db00d`.
- No Supabase migration/schema change, scheduler activation, `pg_cron`, `pg_net`, provider-registry change or deferred capability expansion was part of this rollout. Reconciliation/maintenance endpoints remain implemented but unscheduled unless separately authorized later.

## Post-Cycle 3 Image Thumbnail Storage Contract
- RenderLab continues using the existing private Cloudflare R2 resource and existing nullable `media_assets.thumbnail_storage_key`; no bucket, schema, credential or CDN/public-cache change is introduced.
- Ordinary durable image uploads and generated image outputs best-effort persist WebP previews with a maximum 640px edge at `renderlab/thumbnails/<YYYY>/<MM>/<asset-id>.webp`. The year/month follows the durable primary-object namespace when available.
- Original media remains authoritative. Thumbnail creation failure must not fail upload/generation persistence, and product media routes remain owner-gated before issuing short-lived signed R2 reads.
- Pre-existing active image rows with null thumbnail keys are repaired only by the explicit `scripts/backfill-image-thumbnails.mjs` maintenance command. It is dry-run by default, bounded by `--max`, optionally owner-scoped, idempotent by null-key selection and requires `--apply` before mutating R2/Supabase. It is not a scheduler or request-path background job.
- Post-Cycle 3 stabilization maintenance run `34062569699` executed this contract after PR #112 merged as `7798fab8ad8caff19a74005502eb7472297ba7a8`: 13 active image rows were eligible, all 13 were backfilled, 0 failed and the exact remaining null-thumbnail count was 0. Evidence artifact `9997940257` has artifact ZIP `sha256:bb02409e57ac2ba0737b72fb9cc848d7974bede01ff181f8ac4e36638f7d4e46`.
- This maintenance changed only existing private R2 thumbnail objects and the existing nullable `media_assets.thumbnail_storage_key` values. It did not create a bucket/resource, change schema, redeploy a worker or activate a scheduler. Explicit rollout run `34063455944` subsequently deployed exact application source `71a9034039a64beec66894cc4f79b1f62bfc7bf7` as READY production deployment `dpl_Ck2HEMFpt2aRUwSVTrYA6YcFTbbi` (`https://renderlab-i8mxlfmki-faresmohamed260-6733s-projects.vercel.app`) with `https://renderlab.faresuniform.uk` aliased. The rollout verified production environment metadata, public routes, exact-origin R2 upload CORS, durable image upload, deterministic WebP thumbnail delivery/cache behavior, Library listing and cleanup; rollback to `dpl_6htPrpLMysfqZycZ7wQ5btwejXPA` was not required. Evidence artifact `9998223539` has JSON sha256 `4d4130f6c2a8768b5a4a5f068e1d2d6b182c849ccaf10df229f985c9ebab05fd` and ZIP sha256 `540839b3655089fc9b4c72b7bee0a21280b29dbe377e3b054c295b92737e9bdb`.

## Post-Cycle 3 Image Model Choice — infrastructure evidence
No worker, Modal app, Supabase schema, R2 resource, scheduler or production environment was changed to expose Qwen as a product choice. Existing fleet registrations remain authoritative: the Qwen primary/standby pair stays active and FLUX retains its current active-routing state.

A fresh read-only remote audit on 2026-09-07 (`34064898642`, artifact `9998623751`, `sha256:5075cdef0ee55cfa7b6649642815fcb2d6f585fc27565d0eb85f6bc9907975af`) queried both registered Qwen gateways directly. Both were ready, identified `Qwen/Qwen-Image-Edit-2511`, advertised multi-reference async `/jobs/edit`, GET/DELETE job lifecycle endpoints and the same fixed Lightning 4-step / CFG 1.0 contract. This audit did not submit a generation or mutate worker state.

The repository adds an isolated Image Model Routing test seam: only when `RENDERLAB_TEST_NATIVE_WORKER_OVERRIDE=true` does native submission use `RENDERLAB_TEST_NATIVE_WORKER_GATEWAY_URL` instead of the registered gateway URL. Run `34065515794` used that seam against the existing local mock to validate model→ecosystem/worker selection and durable model intent without spending provider compute. Do not set these test variables in production. This seam is verification plumbing, not an alternate deployment/routing architecture.
...
The user explicitly authorized production rollout after Phase 22 / Cycle 4 repository closure. Guarded GitHub rollout run `34356155380` checked out exact current `main` `cf3923097fce62edbee643df9b2883bd09210046`, required a pristine tree and required production Vercel environment-key metadata before invoking a forced production build/deploy with the established Vercel CLI path.

Vercel created deployment `dpl_5hA4ihp644VcCzioY66hoTXaP18v` at `https://renderlab-91pu92g5z-faresmohamed260-6733s-projects.vercel.app` and reported it `READY` with Git SHA `cf3923097fce62edbee643df9b2883bd09210046`. Active production aliases include `https://renderlab.faresuniform.uk` and `https://renderlab-lake.vercel.app`. The prior READY deployment `dpl_Ck2HEMFpt2aRUwSVTrYA6YcFTbbi` was retained as the immediate rollback target; rollback was armed for any post-deploy failure but was not required.
...
## UI-068 through UI-070 production rollout — 2026-09-10
The user explicitly authorized production deployment of the verified repository state. Exact candidate `378ea68b5c3087f84e83cc73682de2f919168c0a` contains UI-068, UI-069 and UI-070 plus their repository closure documentation; automatic Git → Vercel deployment remains disabled.

Initial guarded rollout run `34427437110` checked out the exact candidate with a clean pre-deploy tree and created READY deployment `dpl_8pg99Zf6KthBTWKpXCjvuFAZdpKu`. Landing plus `/create`, `/library`, `/activity` and `/settings` smoke passed, but Vercel metadata reported `gitDirty=1` because the minimal helper did not prebind the project/team IDs and CLI project-link metadata mutated the runner workspace after the pre-deploy clean check. That deployment was immediately superseded rather than accepted as the definitive release baseline.
...
## #219 account-data lifecycle infrastructure

#219 adds durable account-export/lifecycle state in Supabase and owner-scoped R2 export artifacts under the RenderLab account-export namespace. Export download availability expires logically after 24 hours; the application refuses expired downloads even before physical deletion, and the existing maintenance surface performs physical expiry cleanup. `vercel.json` includes the bounded daily recovery/cleanup schedule allowed by the current deployment plan; this does not authorize or imply a production deployment.

Account deletion derives storage keys from owner-scoped database state plus deterministic generation-output candidates, waits through the six-minute presigned-upload quiescence window, deletes and proves R2 absence, finalizes owner product cleanup transactionally, de-identifies the claimed invitation, and calls the supported Supabase Admin Auth delete boundary last. The configured verifier on exact head `886a4722826b268ad156eda22e817002471cd04c` proved retry convergence for export write, R2 deletion, database finalization, and Auth deletion without cross-account interference. The two trigger-only `SECURITY DEFINER` helpers introduced for lifecycle integrity have RPC execute revoked from `anon`, `authenticated`, and `service_role`; the Security Advisor recheck showed no new #219 warning.

RenderLab-controlled rows and R2 objects are within this deletion proof. Provider/service logs maintained by Supabase, Cloudflare, Vercel, or Modal remain subject to those providers' retention contracts and must not be described as synchronously erased by RenderLab.
