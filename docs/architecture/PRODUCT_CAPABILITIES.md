...
## Phase 18 Image Upscale v0.1 — 18A–18C verified; lifecycle/UI pending
...
Phase 18A–18C now verify the worker, domain/schema and server API/admission prerequisites. `GenerationRequest` remains prompt-generation-only; persisted `CreativeOperation` additionally includes `upscale-image`, and `src/lib/capabilities/upscale.ts` owns the separate fixed-2× promptless command with one durable `media-asset` primary image and canonical `parameters.upscale.scale=2`. Shared migration `20260906004810 renderlab_image_upscale_job_semantics` permits `prompt=NULL` only for Upscale while retaining nonblank prompts for Create/Edit/Video/Animate. The owner-scoped Upscale route now reloads/validates the active same-owner source and submits the native worker under ordinary generation admission without browser-controlled infrastructure identity. This is **server-exposed but not yet end-user exposed**: native lifecycle/finalization/cancellation/failed Retry, truthful non-prompt Activity summary, Compare source and Viewer action remain 18D–18E work. Successful Run Again/Reuse Settings remain absent, historical rows remain unchanged, and Restore remains deferred until a distinct deployed restoration semantic exists.
