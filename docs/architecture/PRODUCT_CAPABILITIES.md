...
Phases 6–9 are complete and verified under the Closed Beta boundary: Phase 7 Create v2, Phase 8 Library v2 and Phase 9 failed-job Retry are implemented and exact-head validated. Phase 10 Account, Admin & Closed-Beta Operations is the current execution-ready contract under UI-051; its implementation begins only after the contract is merged, in the ordered 10A → 10B → 10C → 10D sequence. Verified current Create, Library and Activity behavior above remains authoritative until each Phase 10 slice is implemented and validated.

### Phase 10 Closed-Beta account/admin/guardrail contract — Accepted / not yet implemented
...
- Leaked-password protection is not treated as implemented by app code. Current live Security Advisor still reports it disabled, and current Supabase docs make the control plan-dependent. Phase 10 records/clears the setting when supported; otherwise broader-beta readiness remains blocked.
