...
## Phase 18 Image Upscale v0.1 — COMPLETE / VERIFIED; repository merge pending
...
- product operation: `upscale-image`;
...
Phase 18A–18D now verify the worker, domain/schema, server API/admission and lifecycle/recovery prerequisites. `GenerationRequest` remains prompt-generation-only; persisted `CreativeOperation` additionally includes `upscale-image`, and `src/lib/capabilities/upscale.ts` owns the separate fixed-2× promptless command with one durable `media-asset` primary image and canonical `parameters.upscale.scale=2`. Shared migration `20260906004810 renderlab_image_upscale_job_semantics` permits `prompt=NULL` only for Upscale while retaining nonblank prompts for Create/Edit/Video/Animate. The owner-scoped Upscale route now reloads/validates the active same-owner source and submits the native worker under ordinary generation admission without browser-controlled infrastructure identity. The fixed-2× operation is end-user exposed through the server-eligible Viewer action verified in 18E; Phase 18F product/live proof completed in Image Upscale Integration `34042759032` on exact head `fec2ad2762d21cbd92c8db03c8a95130a6b6ecb7`. Phase 18D now verifies native polling/reconciliation/finalization/cancellation, failed Retry with current-source revalidation, truthful Activity `2× upscale`, and conditional succeeded-result Compare source with tombstoned-source suppression. Successful Run Again/Reuse Settings remain absent, historical rows remain unchanged, and Restore remains deferred until a distinct deployed restoration semantic exists.

### Phase 18E Viewer exposure — Verified in RenderLab
- The existing Media Viewer is the only v0.1 entry point. A source image receives **Upscale 2×** only when current owner-loaded durable metadata is conservatively eligible and the server has configured Upscale backend presence. Unsupported/incomplete metadata omits the action; the 18C route still revalidates stored bytes, MIME and decoded geometry authoritatively before admission.
- The browser sends no Upscale settings/body. A feature-local duplicate lock prevents accidental concurrent clicks; starting, accepted and retryable-error states use maintained product primitives while the source Viewer remains unchanged. Accepted copy routes the user to Activity and does not imply the result is ready.
- Repository-backed v0.3 desktop + 390px design checkpoint precedes the implementation. Exact implementation head `ac4aed60e64061ee6a911c858cdc032b6f9a7423` passed Upscale Viewer Visual `34033506667` plus Generation Integration `34033506663`, Creative Iteration `34033506642`, Reconciliation `34033510954`, Cancellation `34033506603`, UI Shell `34033506628`, Engineering Quality `34033506656` and the remaining real implementation-head regressions. Artifact `9989427506` (`sha256:49e40583899891a1f1863ff8fd49a714febf11bfdb999bb781849f45c3e01121`) was independently hash-checked and human-reviewed clean with no corrective UI change.
- 18E changes no schema, worker deployment, provider routing, production app deployment or scheduler state. Phase 18F subsequently completed the dedicated product/live proof without changing those boundaries.

### Phase 18F dedicated product/live proof — VERIFIED
- `34042759032` passed the complete run-owned matrix plus one real request through the already-deployed Modal worker: one slot-0 PNG, exact `8×6 → 16×12`, source preserved, `renderlab-upscale-01` / `image-upscale-v1`.
...
- Exact proof head closed at 32/32 attached workflows successful. Phase 18 is therefore complete/verified in the repository candidate; merge and production rollout remain separate.
