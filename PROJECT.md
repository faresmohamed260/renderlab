...
- Authoritative repository source currently deployed is `378ea68b5c3087f84e83cc73682de2f919168c0a`. It includes the verified UI-068 / UI-069 / UI-070 post-production corrections plus their repository closure documentation.
...
- Accepted production application is source `378ea68b5c3087f84e83cc73682de2f919168c0a` at READY deployment `dpl_DoNm8T21WSano7zoWgGi112sDDqp` / `https://renderlab-fc4hli5nz-faresmohamed260-6733s-projects.vercel.app`; `https://renderlab.faresuniform.uk` and the stable Vercel alias serve that deployment. Prior accepted READY deployment `dpl_BpMCWYggKkzf8FuWpb2vLun46r3M` remains the preferred rollback anchor.
...
- Production is verified through UI-070. UI-068 / PR #138, UI-069 / PR #140 and UI-070 / PR #142 are production-live in exact source `378ea68b5c3087f84e83cc73682de2f919168c0a` at READY deployment `dpl_DoNm8T21WSano7zoWgGi112sDDqp`. Guarded clean rollout run `34427597654` preserved a pristine working tree before and after deploy, passed Landing + `/create` + `/library` + `/activity` + `/settings` smoke, and required no rollback. Automatic Git → Vercel deployment remains disabled.
- UI-071 / PR #145 is merged as `be0d2fa23fd16acbd5202e88ba2a09a7d8eefc20`: desktop application routes omit the redundant top context bar, mobile/narrow routes retain account/Settings access, and Create reference-drop copy aligns beside its icon. Exact candidate `c151b0284a29a469b1e89d5f4cd66c55151d1d01` passed all 14 attached workflows and responsive rendered review; all four workflows attached to merged main also passed. UI-071 has not been production deployed. See `docs/ui/UI_MIGRATION.md` for the verification record.
