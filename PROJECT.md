...
## Current Priority
**Cycle 2 — Creative Productivity & Beta Maturity. Phase 6 is complete under the Closed Beta boundary and Phase 7 Create v2 / Creative Direction is complete and exact-head verified. Phase 8 Library v2 / Media Workflow Productivity is now expanded/planned from current repository reality; implementation has not started. The first implementation slice is Phase 8A Collection Management.**
...
- **Phase 8 — Library v2 / Media Workflow Productivity:** execute in two ordered slices under UI-049. **8A Collection Management** brings the existing owner-scoped collection create capability into a Library-owned management surface and adds collection rename/delete without deleting media. **8B Page-scoped Batch Organization** extends the existing UI-034 current-page selection model with explicit Favorite/Unfavorite and Add/Remove Collection target-state actions. The existing owner-enforced `media_collections` / `media_collection_items` / `media_assets.favorited_at` schema is sufficient at planning time, so no migration is expected. Cross-page selection, Trash/restore, a Collections destination and a global media client store remain deferred.
...
- Current phase contract: **Phase 8 — Library v2 / Media Workflow Productivity**, `EXPANDED/PLANNED`; execution `NOT STARTED`.
- Current product slice: **Phase 8A Collection Management is next.** UI-049 locks Library-owned collection management plus the Phase 8B reuse of UI-034 page-scoped selection for explicit Favorite/Unfavorite and Add/Remove Collection actions. Planning evidence shows the existing owner-enforced schema is sufficient; any implementation-time discovery that requires a migration must stop and amend the contract rather than silently broadening scope. This contract merge does not authorize deployment.
