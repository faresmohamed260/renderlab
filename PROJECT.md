...
## Current Priority
**Cycle 2 — Creative Productivity & Beta Maturity. Phases 6–8 are complete and exact-head verified. Phase 9 Activity v2 / Recovery & Job Control is expanded under UI-050: failed-job Retry v0.1 is the next implementation slice; Cancel remains deferred because the current orchestration lacks cancellation-safe race guards; the optional shell attention indicator is also deferred.**
...
- **Phase 9 — Activity v2 / Recovery & Job Control: EXPANDED / READY FOR EXECUTION.** UI-050 makes failed-job **Retry v0.1** the product slice: retry creates a distinct new owner-scoped job from stored normalized product intent only after current request/capability/input revalidation, never from historical worker/provider payloads. The cancellation safety audit found a concrete orchestration blocker—poll-time reassignment/result persistence can race a local cancellation because there is no cancellation-aware compare-and-set/lease guard—so **no Cancel product mutation/UI is approved**. The optional shell attention badge is deferred because recovery does not justify a new cross-route polling/global-status layer yet.
...
- Current completed phase contract: **Phase 8 — Library v2 / Media Workflow Productivity**, `EXPANDED/COMPLETE`; execution `COMPLETE / VERIFIED`.
- Current phase contract: **Phase 9 — Activity v2 / Recovery & Job Control**, `EXPANDED / READY FOR EXECUTION`; execution `NOT STARTED` until this contract merge. UI-050 authorizes failed-job Retry v0.1 only; Cancel and shell-global attention remain deferred.
- Current product slice: **Phase 9A Generation Retry v0.1 is next after the Phase 9 contract merge.** Retry is failed-job-only, owner-scoped and creates a new ordinary generation job after revalidating stored product intent against current capability and current source availability. Original jobs remain immutable historical records. Explicit repeated Retry requests may create separate attempts; the UI prevents accidental concurrent double-submit with a busy state. No deployment is authorized by contract acceptance.
