...
**Status: `COMPLETE / VERIFIED THROUGH PHASE 18; REPOSITORY MERGE PENDING`.**
...
- The public Modal gateway committed in the dedicated CI workflow is a verification fallback only. Production RenderLab still requires server-only `RENDERLAB_UPSCALE_WORKER_GATEWAY_URL`; no Vercel deployment/environment, worker deployment, scheduler, `pg_cron` or `pg_net` state changed.
- PR #111 remains draft, mergeable and unmerged with a clean review surface. Merge and production rollout remain separate explicit operations.
...
**Status: `COMPLETE / VERIFIED — REPOSITORY MERGE PENDING`.**
**UI decision:** UI-058.
